<script lang="ts">
  ...
  function getCountryCode(element: HTMLElement): string | null {
    ...
    const dataCode = element.getAttribute('data-country-code');
    if (dataCode) {
      return dataCode.toUpperCase();
    }

    // Fallback to ID (for backward compatibility)
    if (element.id && element.id.length === 2) {
      return element.id.toUpperCase();
    }
  ...
  function click(event: MouseEvent) {
    ...
    const code = getCountryCode(target);
  ...
  function handleMouseEnter(event: MouseEvent) {
    const target = event.target as HTMLElement;
    // Only process if target is a path element
...
>
  ...
  <path d="M1953.8 456l1 0.4-0.2 0-0.8-0.4z" id="MH" name="Marshall Islands" data-country-code="MH">
  </path>
  <path
    class="Malta"
  ...
  <path d="M1933 505.3l-0.2 0-0.1-0.3 0.3 0 0 0.3z" id="NR" name="Nauru" data-country-code="NR">
  </path>
  <path
  ...
  <path d="M1998.9 556.6l0-0.2 0.1 0 0 0.1-0.1 0.1z" id="TV" name="Tuvalu" data-country-code="TV">
  </path>
  <path
...
<style>
  ...
  /* Default hover effect for all paths */
  svg path {
    transition:
      fill 0.2s ease,
  ...
  /* Selected countries - using lowercase class names as generated by the component */
  /* Americas */
  :global(svg.selected-ca path.Canada),
  :global(svg.selected-us path.United.States),
  :global(svg.selected-mx path[name='Mexico']),
  :global(svg.selected-br path#BR),
  :global(svg.selected-ar path.Argentina),
  :global(svg.selected-cl path#CL),
  :global(svg.selected-pe path#PE),
  :global(svg.selected-co path#CO),
  :global(svg.selected-ve path#VE),
  :global(svg.selected-ec path#EC),
  :global(svg.selected-bo path#BO),
  :global(svg.selected-py path#PY),
  :global(svg.selected-uy path#UY),
  :global(svg.selected-gy path#GY),
  :global(svg.selected-sr path#SR),
  :global(svg.selected-gf path#GF),

  /* Europe */
  :global(svg.selected-fr path.France),
  :global(svg.selected-gb path.United.Kingdom),
  :global(svg.selected-de path#DE),
  :global(svg.selected-es path#ES),
  :global(svg.selected-it path.Italy),
  :global(svg.selected-be path#BE),
  :global(svg.selected-nl path#NL),
  :global(svg.selected-ch path#CH),
  :global(svg.selected-at path#AT),
  :global(svg.selected-pt path#PT),
  :global(svg.selected-lu path#LU),
  :global(svg.selected-hr path#HR),
  :global(svg.selected-ie path#IE),
  :global(svg.selected-no path#NO),
  :global(svg.selected-se path#SE),
  :global(svg.selected-fi path#FI),
  :global(svg.selected-dk path#DK),
  :global(svg.selected-pl path#PL),
  :global(svg.selected-cz path#CZ),
  :global(svg.selected-sk path#SK),
  :global(svg.selected-hu path#HU),
  :global(svg.selected-ro path#RO),
  :global(svg.selected-bg path#BG),
  :global(svg.selected-gr path#GR),
  :global(svg.selected-hr path#HR),
  :global(svg.selected-si path#SI),
  :global(svg.selected-rs path#RS),
  :global(svg.selected-ba path#BA),
  :global(svg.selected-al path#AL),
  :global(svg.selected-mk path#MK),
  :global(svg.selected-me path#ME),
  :global(svg.selected-xk path#XK),
  :global(svg.selected-ee path#EE),
  :global(svg.selected-lv path#LV),
  :global(svg.selected-lt path#LT),
  :global(svg.selected-by path#BY),
  :global(svg.selected-ua path#UA),
  :global(svg.selected-md path#MD),
  :global(svg.selected-ru path#RU),
  :global(svg.selected-is path#IS),
  :global(svg.selected-lu path#LU),
  :global(svg.selected-li path#LI),
  :global(svg.selected-mt path#MT),
  :global(svg.selected-cy path#CY),

  /* Asia */
  :global(svg.selected-jp path#JP),
  :global(svg.selected-cn path#CN),
  :global(svg.selected-in path#IN),
  :global(svg.selected-kr path#KR),
  :global(svg.selected-kp path#KP),
  :global(svg.selected-tw path#TW),
  :global(svg.selected-hk path#HK),
  :global(svg.selected-mo path#MO),
  :global(svg.selected-mn path#MN),
  :global(svg.selected-kz path#KZ),
  :global(svg.selected-uz path#UZ),
  :global(svg.selected-tm path#TM),
  :global(svg.selected-kg path#KG),
  :global(svg.selected-tj path#TJ),
  :global(svg.selected-af path#AF),
  :global(svg.selected-pk path#PK),
  :global(svg.selected-bd path#BD),
  :global(svg.selected-np path#NP),
  :global(svg.selected-bt path#BT),
  :global(svg.selected-lk path#LK),
  :global(svg.selected-mm path#MM),
  :global(svg.selected-th path#TH),
  :global(svg.selected-la path#LA),
  :global(svg.selected-vn path#VN),
  :global(svg.selected-kh path#KH),
  :global(svg.selected-my path.Malaysia),
  :global(svg.selected-sg path#SG),
  :global(svg.selected-id path#ID),
  :global(svg.selected-ph path.Philippines),
  :global(svg.selected-tl path#TL),
  :global(svg.selected-bn path#BN),

  /* Middle East */
  :global(svg.selected-sa path#SA),
  :global(svg.selected-ae path#AE),
  :global(svg.selected-qa path#QA),
  :global(svg.selected-kw path#KW),
  :global(svg.selected-bh path#BH),
  :global(svg.selected-om path#OM),
  :global(svg.selected-ye path#YE),
  :global(svg.selected-iq path#IQ),
  :global(svg.selected-ir path#IR),
  :global(svg.selected-sy path#SY),
  :global(svg.selected-jo path#JO),
  :global(svg.selected-lb path#LB),
  :global(svg.selected-il path#IL),
  :global(svg.selected-ps path#PS),
  :global(svg.selected-tr path#TR),
  :global(svg.selected-am path#AM),
  :global(svg.selected-az path.Azerbaijan),
  :global(svg.selected-ge path#GE),

  /* Africa */
  :global(svg.selected-za path#ZA),
  :global(svg.selected-eg path#EG),
  :global(svg.selected-ng path#NG),
  :global(svg.selected-ke path#KE),
  :global(svg.selected-tn path#TN),
  :global(svg.selected-ma path#MA),
  :global(svg.selected-dz path#DZ),
  :global(svg.selected-ly path#LY),
  :global(svg.selected-sd path#SD),
  :global(svg.selected-et path#ET),
  :global(svg.selected-ug path#UG),
  :global(svg.selected-tz path#TZ),
  :global(svg.selected-ao path.Angola),
  :global(svg.selected-zm path#ZM),
  :global(svg.selected-zw path#ZW),
  :global(svg.selected-bw path#BW),
  :global(svg.selected-mz path#MZ),
  :global(svg.selected-na path#NA),
  :global(svg.selected-cd path#CD),
  :global(svg.selected-cg path#CG),
  :global(svg.selected-cm path#CM),
  :global(svg.selected-ci path#CI),
  :global(svg.selected-gh path#GH),
  :global(svg.selected-sn path#SN),
  :global(svg.selected-ml path#ML),
  :global(svg.selected-ne path#NE),
  :global(svg.selected-bf path#BF),
  :global(svg.selected-gn path#GN),
  :global(svg.selected-sl path#SL),
  :global(svg.selected-lr path#LR),
  :global(svg.selected-tg path#TG),
  :global(svg.selected-bj path#BJ),
  :global(svg.selected-rw path#RW),
  :global(svg.selected-bi path#BI),
  :global(svg.selected-so path#SO),
  :global(svg.selected-er path#ER),
  :global(svg.selected-dj path#DJ),
  :global(svg.selected-ss path#SS),
  :global(svg.selected-cf path#CF),
  :global(svg.selected-td path#TD),
  :global(svg.selected-ga path#GA),
  :global(svg.selected-gq path#GQ),
  :global(svg.selected-mr path#MR),
  :global(svg.selected-gm path#GM),
  :global(svg.selected-gw path#GW),
  :global(svg.selected-mg path#MG),
  :global(svg.selected-mu path#MU),
  :global(svg.selected-km path#KM),
  :global(svg.selected-sc path#SC),
  :global(svg.selected-cv path#CV),
  :global(svg.selected-st path#ST),
  :global(svg.selected-ls path#LS),
  :global(svg.selected-sz path#SZ),

  /* Oceania */
  :global(svg.selected-au path.Australia),
  :global(svg.selected-nz path.New-Zealand),
  :global(svg.selected-pg path#PG),
  :global(svg.selected-fj path.Fiji),
  :global(svg.selected-sb path#SB),
  :global(svg.selected-nc path#NC),
  :global(svg.selected-pf path#PF),
  :global(svg.selected-vu path#VU),
  :global(svg.selected-ws path#WS),
  :global(svg.selected-ki path#KI),
  :global(svg.selected-to path#TO),
  :global(svg.selected-fm path#FM),
  :global(svg.selected-pw path#PW),
  :global(svg.selected-mh path#MH),
  :global(svg.selected-nr path#NR),
  :global(svg.selected-lb path#LB),
  :global(svg.selected-ro path#RO),
  :global(svg.selected-th path#TH),
  :global(svg.selected-sg path#SG),
  ...
  :global(svg.selected-gb path.United.Kingdom),
  :global(svg.selected-it path.Italy),
  :global(svg.selected-tv path#TV) {
